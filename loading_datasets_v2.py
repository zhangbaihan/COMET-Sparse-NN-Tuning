...
def get_data_loaders(dataset_name, batch_size, data_augmentation=False, return_indices=False):
    """
    ...
    Args:
        ...
        return_indices (bool): Whether to return indices along with data and target.
    ...
    if dataset_name == 'cifar10':
        ...
        train_set, val_set = torch.utils.data.random_split(full_train_dataset, [train_size, val_size], generator=generator)
        ...
        val_set = test_dataset # SARCOS uses test as val in original code, keep as is or split? Original returned test_loader=None
        test_set = test_dataset # Reuse for test
        num_classes = 1
        
        if return_indices:
             # TensorDataset doesn't support wrapping nicely with current design if we want just indices. 
             # But IndexedDataset wrapper works for any map-style dataset.
             pass
    ...
    train_loader = DataLoader(train_set, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True, persistent_workers=True, prefetch_factor=4)
    val_loader = DataLoader(val_set, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True, persistent_workers=True, prefetch_factor=4)
    
    if dataset_name == 'SARCOS':
        test_loader = None # Maintain original behavior for SARCOS or fix? Original had test_loader=None
    else:
        test_loader = DataLoader(test_set, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True, persistent_workers=True, prefetch_factor=4)
