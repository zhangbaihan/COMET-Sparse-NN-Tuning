...
def get_COMET_normalized(dataset_name, layer_1_neurons, layer_2_neurons, layer_3_neurons, layer_4_neurons,
                      topk_rate, norm, activation):
    """
    Returns a model implementing COMET routing (conditionally-overlapping routing function).
    With normalized input to the first router.
    """
    ...
    class SpecModel(nn.Module):
        ...
        def forward(self, x):
            ...
            x_nn = self.act(x_nn)
            with torch.no_grad():
                x_router = x / (x.norm(dim=-1, keepdim=True) + 1e-6)
            ...
            with torch.no_grad():
                ...
                # Apply normalization for spec_2 as well?
                # The user said "add this to routers" (plural).
                # And spec_2 takes x_spec1 * mask1 as input.
                # Let's normalize that input too, assuming consistency.
                
                router_input_2 = x_spec1 * mask1
                router_input_2 = router_input_2 / (router_input_2.norm(dim=-1, keepdim=True) + 1e-6)
                x_spec2 = self.spec_2(router_input_2)
            ...
            with torch.no_grad():
                ...
                # Apply normalization for spec_3 as well
                router_input_3 = x_spec2 * mask2
                router_input_3 = router_input_3 / (router_input_3.norm(dim=-1, keepdim=True) + 1e-6)
                x_spec3 = self.spec_3(router_input_3)
