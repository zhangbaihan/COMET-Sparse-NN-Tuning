...
for j, neurons in enumerate(neurons_list):
    for i, topk_rate in enumerate(topks):
        ...
        for k, model_name in enumerate(model_names):
            file_path = f'results/{model_name}/{activation}/{optimizer_name}/lr_0.001/{dataset_name}_topk_{topk_rate}_neurons_{neurons}'
            try:
                with open(file_path, 'rb') as fp:
                    model_results = pickle.load(fp) 
                val_acc_key = f'{model_name}_val_acc'
                max_seed, min_seed, avg_variance = get_max_and_min_arrays(model_results[val_acc_key])
                ...
                print(f"Warning: Results not found for {model_name} with neurons={neurons}, topk={topk_rate}")
                continue
