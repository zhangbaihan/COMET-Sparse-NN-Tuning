...
def parse_args():
    parser = argparse.ArgumentParser(description="Train TopK Scheduler Model")
    ...
    parser.add_argument("--topk", type=float, default=0.1, help="Final Target Sparsity rate")
    parser.add_argument("--activation", type=str, default="softplus")
    parser.add_argument("--norm", type=str, default=None, choices=[None, "batch", "layer"])
    
    # Scheduler Specifics
    parser.add_argument("--step_every", type=int, default=10, help="Update sparsity every N epochs")
    parser.add_argument("--step_pk", type=float, default=-0.1, help="Amount to decrease sparsity by")
    
    # Saving
    parser.add_argument("--save_dir", type=str, default="experiments_Scheduler", help="Directory to save results")
...
def main():
    ...
    print(f"Starting TopK Scheduler training on {args.dataset}...")
    print(f"Device: {device}")
    
    # Create save directory
    run_name = f"{args.dataset}_neurons{args.neurons}_targetTopk{args.topk}_lr{args.lr}_seed{args.seed}"
    ...
    for epoch in range(args.epochs):
        ...
        history['top_k'].append(current_topk)
        history['epoch_times'].append(duration)
        
        print(f"Epoch {epoch+1}/{args.epochs} | "
              f"Train Loss: {t_loss:.4f} Acc: {t_acc:.4f} | "
              f"Val Loss: {v_loss:.4f} Acc: {v_acc:.4f} | "
              f"TopK: {current_topk:.2f} | Time: {duration:.2f}s")
