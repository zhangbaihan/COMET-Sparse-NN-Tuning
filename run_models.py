...
def init_model(model_name, seed, dataset_name, layer_sizes, topk_rate, norm, activation,
    ...
    model_map = {
        ...
        'COMET_affine': lambda: get_COMET_affine(dataset_name, layer_1, layer_2, layer_3, layer_4, topk_rate, norm, activation),
    }
...
def main():
    ...
    train_loader, val_loader, test_loader, num_classes = get_data_loaders(args.dataset, args.batch_size, False, split_seed=args.data_seed)
    ...
    if args.models is not None:
        # Keep only requested models that exist in the predefined list
        model_names = [m for m in args.models if m in set(model_names)]
        if len(model_names) == 0:
            raise ValueError("No valid models specified with --models")
    ...
    for seed_epoch in range(args.seeds):
        ...
        for epoch in range(args.epochs):
            ...
            with torch.no_grad():
                ...
                # Save best checkpoint per seed (optional)
                if args.save_checkpoints and val_accuracies[-1] > best_val_acc:
                    best_val_acc = val_accuracies[-1]
                    ckpt_path = f"{args.checkpoint_dir}/{args.activation}/{args.optimizer}/lr_{args.lr}/{args.dataset}_topk_{topk_rate}_neurons_{layer_sizes[0]}/{model_name}/seed_{seed}.pth"
